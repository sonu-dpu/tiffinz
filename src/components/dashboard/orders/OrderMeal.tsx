...
const formSchema = z.object({
  ...
  mealFor: z.enum([DailyMealFor.breakfast, DailyMealFor.morning, DailyMealFor.dinner]),
  description: z.string().optional(),
});

type FormValues = z.infer<typeof formSchema>;

export default function OrderMeal({ className, onSuccess }: { className?: string, onSuccess?: () => void }) {
  ...
  const availableCategories = ["ALL", ...Array.from(new Set(meals.map(m => m.type)))];
  const filteredMeals = meals.filter(m => activeCategory === "ALL" || m.type === activeCategory);
        ...
        {showOrderForm && selectedMeal ? (
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>Fill in the timing and details for your selected meal.</CardDescription>
            </div>
          </div>
        ) : (
          <div>
            <CardTitle className="text-xl font-bold">Select a Meal</CardTitle>
            <CardDescription>Browse through our options to get started.</CardDescription>
          </div>
        )}
      </CardHeader>
      <CardContent>
        {!showOrderForm ? (
          <div className="space-y-6">
            ...
            {/* Meals Grid */}
            {loadingMeals ? (
              <div className="text-sm text-muted-foreground animate-pulse text-center py-8">Loading meals...</div>
            ) : meals.length === 0 ? (
              <div className="text-sm text-red-500 text-center py-8">No active meals available.</div>
            ) : filteredMeals.length === 0 ? (
              <div className="text-sm text-muted-foreground text-center py-8">No meals found in this category.</div>
            ) : (
                    ...
                    >
                      ...
                      <div>
                        <div className="flex justify-between items-start mb-1">
                           <h3 className="font-semibold text-lg max-w-[85%] leading-tight">{meal.name}</h3>
                        </div>
                      ...
                      <div className="mt-auto pt-3 flex items-center justify-between">
                        ...
                        <Button size="sm" variant={isSelected ? "default" : "secondary"} className="h-8 rounded-full px-4 text-xs">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 slide-in-from-right-4 animate-in duration-300">
            ...
            {selectedMeal && (
              <div className="p-4 bg-muted/50 rounded-xl border border-border flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{selectedMeal.type} • ₹{selectedMeal.price}</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="block mb-2 text-sm font-medium text-foreground text-left">
                  Time
                  <Select 
                    defaultValue={DailyMealFor.morning} 
                    onValueChange={(value) => setValue("mealFor", value as any)}
                  >
                    <SelectTrigger className={cn("w-full mt-1", errors.mealFor && "border-red-500")}>
                      <SelectValue placeholder="Select time" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value={DailyMealFor.breakfast}>Breakfast</SelectItem>
                      <SelectItem value={DailyMealFor.morning}>Lunch</SelectItem>
                      <SelectItem value={DailyMealFor.dinner}>Dinner</SelectItem>
                    </SelectContent>
                  </Select>
                </label>
                {errors.mealFor && <span className="text-red-600 text-sm">{errors.mealFor.message}</span>}
              </div>
            </div>
            ...
            <div className="pt-4 flex justify-end gap-3 border-t mt-6 pt-6">
              <Button type="button" variant="ghost" onClick={() => setShowOrderForm(false)}>
                Back
              </Button>
              <LoaderButton
